...
from c left join http_api.purchase_event as d on c.user_id=d.user_id
group by 1,2),
...
web AS (
            ...
            fp as
            (select user_id,
              sum(case WHEN metadata_series_name LIKE '%Heartland%' and date(timestamp)>date_sub(current_date(),interval 30 day) then duration else 0 end) as heartland_recent,
              sum(case WHEN metadata_series_name LIKE '%Heartland%' and date(timestamp)<=date_sub(current_date(),interval 30 day) then duration else 0 end) as heartland_older,
              sum(case WHEN metadata_series_name LIKE '%Bates%' and date(timestamp)>date_sub(current_date(),interval 30 day) then duration else 0 end) as bates_recent,
              sum(case WHEN metadata_series_name LIKE '%Bates%' and date(timestamp)<=date_sub(current_date(),interval 30 day) then duration else 0 end) as bates_older,
              sum(case WHEN metadata_series_name not LIKE '%Heartland%' and metadata_series_name LIKE '%Bates%' and date(timestamp)>date_sub(current_date(),interval 30 day) then duration else 0 end) as other_recent,
              sum(case WHEN metadata_series_name not LIKE '%Heartland%' and metadata_series_name LIKE '%Bates%' and date(timestamp)<=date_sub(current_date(),interval 30 day) then duration else 0 end) as other_older
            from all1
            group by 1),
...
from d left join awl on d.user_id=awl.user_id

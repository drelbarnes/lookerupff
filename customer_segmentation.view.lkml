view: customer_segmentation {
  derived_table: {
    sql: with a as
      ...
      (select a.*,
             case when f.user_id is null then 0 else 1 end as resubscriber,
             case when f.user_id is null then date_diff(current_date(),created_at,day) else tenure+date_diff(current_date(),a.status_date,day) end as tenure_days
      from current_customers as a left join f on a.user_id=f.user_id left join e1 on a.user_id=e1.user_id
      union all
      select a.*,
             0 as resubscriber,

...
with heartland as
(select 231517 as video_id union all
...
android as
(select user_id,
        case when video_id in (select * from heartland) then 1 else 0 end as watched_heartland,
        'android' as platform
 from android.play),
 ...
 web as
 (select user_id,
         case when title ~* 'heartland' then 1 else 0 end as watched_heartland,
         'web' as platform
  from javascript.play),

  play as

  ((select user_id, sum(watched_heartland) as watched_heartland from android group by user_id)
  union all
  (select user_id, sum(watched_heartland) as watched_heartland from ios group by user_id)
  union all

view: marketing_attribution_test {
  derived_table: {
    ...
    sql:
      ...
      FROM (
      SELECT user_id, context_ip, anonymous_id, received_at AS event_received_at, order_id, CAST(NULL AS VARCHAR(255)) AS bundle_plan, CAST('standard' AS VARCHAR(20)) AS trial_type FROM javaScript_upentertainment_checkout.order_completed WHERE received_at >= (CURRENT_DATE - INTERVAL '90 days')::DATE AND received_at < CURRENT_DATE + INTERVAL '1 day' AND user_id IS NOT NULL AND brand = 'upfaithandfamily'
      UNION ALL
      ...
      FROM (SELECT user_id, context_ip, anonymous_id, order_id, received_at AS event_received_at, bundle_plan, CAST('standard' AS VARCHAR(20)) AS trial_type, ROW_NUMBER() OVER (PARTITION BY user_id, order_id ORDER BY CASE WHEN bundle_plan IS NOT NULL THEN 0 ELSE 1 END, received_at ASC) AS dedup_rank FROM (SELECT user_id, context_ip, anonymous_id, received_at, order_id, CAST(NULL AS VARCHAR(255)) AS bundle_plan FROM javaScript_upentertainment_checkout.order_completed WHERE received_at >= (CURRENT_DATE - INTERVAL '90 days')::DATE AND user_id IS NOT NULL AND brand='upfaithandfamily' UNION ALL SELECT user_id, context_ip, anonymous_id, received_at, order_id, bundle_plan FROM javaScript_upentertainment_checkout.order_updated WHERE received_at >= (CURRENT_DATE - INTERVAL '90 days')::DATE AND user_id IS NOT NULL AND brand='upfaithandfamily' AND bundle_plan IS NOT NULL) le_dc) r3 WHERE r3.dedup_rank = 1
      UNION ALL

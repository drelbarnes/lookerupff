...
FROM result
UNION ALL
SELECT report_date, 'yearly'  AS billing_period, total_paid_subs_yearly  AS paid_subscribers
FROM result;;
  }
  dimension: date {
    type: date
  ...
  dimension: paid_subscribers {
    type:  number
    sql: ${TABLE}.tpaid_subscribers;;
  }
  measure: total_paid_subs_yearly {
    type:  sum
    sql: ${TABLE}.paid_subscribers;;
    filters: [billing_period: "yearly"]
  }

  measure: total_paid_subs_monthly {
    type:  sum
    sql: ${TABLE}.paid_subscribers ;;
    filters: [billing_period: "monthly"]

view: upff_multi_platform_attribution {
  derived_table: {
   ...
   WITH meta AS (
    SELECT
      ...
      clicks,
      COALESCE(
        SAFE_CAST(date_start AS DATE),
        SAFE.PARSE_DATE('%Y-%m-%d', SAFE_CAST(date_start AS STRING)),
        SAFE.PARSE_DATE('%Y/%m/%d', SAFE_CAST(date_start AS STRING)),
        SAFE.PARSE_DATE('%m/%d/%Y', SAFE_CAST(date_start AS STRING))
      ) AS date_start,
      COALESCE(
        SAFE_CAST(date_stop AS DATE),
        SAFE.PARSE_DATE('%Y-%m-%d', SAFE_CAST(date_stop AS STRING)),
        SAFE.PARSE_DATE('%Y/%m/%d', SAFE_CAST(date_stop AS STRING)),
        SAFE.PARSE_DATE('%m/%d/%Y', SAFE_CAST(date_stop AS STRING))
      ) AS date_stop,
      frequency,
      impressions,
  ...
  google_ads AS (
    SELECT
      ...
      CAST(NULL AS INT64) AS frequency,
      impressions,
      engagements,
      ...
      CAST(NULL AS INT64) AS social_spend
  ...
  apple_search AS (
    SELECT
      ...
      COALESCE(
        ...
        SAFE.PARSE_DATE('%Y-%m-%d', SAFE_CAST(date AS STRING)),
        SAFE.PARSE_DATE('%Y/%m/%d', SAFE_CAST(date AS STRING)),
        SAFE.PARSE_DATE('%m/%d/%Y', SAFE_CAST(date AS STRING))
      ) AS date_start,
      COALESCE(
        SAFE_CAST(date AS DATE),
        SAFE.PARSE_DATE('%Y-%m-%d', SAFE_CAST(date AS STRING)),

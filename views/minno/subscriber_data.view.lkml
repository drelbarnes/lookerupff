view: subscriber_data {
  ...
  WHERE content_invoice_dunning_status = 'in_progress'
    AND timestamp >= current_date - interval '13 day'  and content_subscription_subscription_items_0_item_price_id like '%Mi%'
  GROUP BY email
),
latest_succeeded AS (
  SELECT content_customer_email as email, MAX(timestamp) AS last_success_time
  FROM chargebee_webhook_events.payment_succeeded
  WHERE timestamp >= current_date - interval '14 day'  and content_invoice_dunning_attempts not like '%attempt%'     AND content_customer_email IS NOT NULL and content_subscription_subscription_items_0_item_price_id like '%Mi%'
      ...
      where subscription_subscription_items_0_item_price_id like '%Mi%' and date(timestamp) = CURRENT_DATE
      ),

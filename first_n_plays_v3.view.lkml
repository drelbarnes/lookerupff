  ...
  derived_table: {
    sql:
       with
        ...
        first_plays as
        (
        select collection, count(distinct user_id) as play_count
        ...
        plays_first_three as
        (
        select collection, count(distinct user_id) as play_count
        from plays_less_granular
        where play_number in (1,2,3)
        and timestamp between {% date_start date_filter %} and {% date_end date_filter %}
        group by 1
        ),
    ...
    parameter: table_name {
      ...
      default_value: "plays_first_three"

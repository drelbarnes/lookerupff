view: series_title_valuation {
    derived_table: {
      sql:
        --series title valuation analysis v1.0 4-3-2023
        ...
        *
        ...
        where date(timestamp) >= {% date_start date_filter %}
        and date(timestamp) <= {% date_end date_filter %}
        ),
        ...
        select *
        from user_first_play
        where date(timestamp) >= l_begin_dt
        and date(timestamp) <= l_end_dt
        ),
        ...
        c.total_ltv,
        round(100 * a.avg_cr + 0.01 * b.number_first_plays + log(c.total_ltv), 0) as value_score

view: all_play {
  derived_table: {
    sql: (with a as
(select a.timestamp,trim(upper(title)) as title,user_id,'Android' as platform from android.play as a left join svod_titles.title_id_mapping as b on a.video_id=b.id
         union all
